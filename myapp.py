...
app.config['SECRET_KEY'] = '123456791'
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:root@localhost/eat_what'
...
@app.route('/list')
def list():
    foods=Food.query.order_by(Food.name.desc()).all()

    return json.dumps(foods, cls=AlchemyEncoder)
...
def build_sample_db():
    ...
    for i in range(1,100):
        ...
        food.name='店家'+str(i)
        food.price=14+i
        food.taste='taste'+str(i)
        food.avatar='https://s3.amazonaws.com/uifaces/faces/twitter/sachagreif/128.jpg'
        db.session.add(food)
